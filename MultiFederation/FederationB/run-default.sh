...
cd $root_directory
xterm -fg white -bg black -l -lf $logs_directory/federation-manager-${timestamp}.log -T "Federation Manager" -geometry 140x40+120+120 -e "export CPSWT_ROOT=`pwd` && mvn exec:java -P FederationManager" &
...
cd $root_directory
xterm -fg white -bg black -l -lf $logs_directory/User-${timestamp}.log -T "User" -geometry 140x40+180+150 -e "mvn exec:java -P ExecJava,User" &
waitUntilJoined User 1

cd $code_directory/Common/target
cp -r $code_directory/Common/conf/ .
xterm -fg yellow -bg black -l -lf $logs_directory/common-federate-${timestamp}.log -T "Common Federate" -geometry 140x40+240+180 -e "java -Dlog4j.configurationFile=conf/log4j2.xml -Djava.net.preferIPv4Stack=true -jar common-federate-0.0.1-SNAPSHOT.jar conf/FederationA.json conf/FederationB.json" &
waitUntilJoined Generator 1
