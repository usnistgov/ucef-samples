...
#include "FrenchTransferReceipt.hpp"




void FrenchTransferReceipt::init( RTI::RTIambassador *rti ) {
	...
	while( isNotInitialized ) {
		try {
			getHandle() = rti->getInteractionClassHandle( "InteractionRoot.C2WInteractionRoot.FrenchTransferReceipt" );
	...
	getClassNameHandleMap().insert(  std::make_pair( "FrenchTransferReceipt", get_handle() )  );
	getClassHandleNameMap().insert(  std::make_pair( get_handle(), "FrenchTransferReceipt" )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "FrenchTransferReceipt,id", get_id_handle() )  );
...
void FrenchTransferReceipt::publish( RTI::RTIambassador *rti ) {
	if ( getIsPublished() ) {
		return;
	}
...
void FrenchTransferReceipt::unpublish( RTI::RTIambassador *rti ) {
	if ( !getIsPublished() ) {
		return;
	}
...
void FrenchTransferReceipt::subscribe( RTI::RTIambassador *rti ) {
	if ( getIsSubscribed() ) {
		return;
	}
...
void FrenchTransferReceipt::unsubscribe( RTI::RTIambassador *rti ) {
	if ( !getIsSubscribed() ) {
		return;
	}
...
bool FrenchTransferReceipt::static_init( void ) {
	static bool isInitialized = false;
	if ( isInitialized ) {
		return true;
	}
	...
	getClassNameSet().insert( "FrenchTransferReceipt" );
	
	getClassNameFactoryMap().insert(  std::make_pair( "FrenchTransferReceipt", &FrenchTransferReceipt::factory )  );
	getClassNamePublishMap().insert(   std::make_pair(  "FrenchTransferReceipt", (PubsubFunctionPtr)( &FrenchTransferReceipt::publish )  )   );
	getClassNameUnpublishMap().insert(   std::make_pair(  "FrenchTransferReceipt", (PubsubFunctionPtr)( &FrenchTransferReceipt::unpublish )  )   );
	getClassNameSubscribeMap().insert(   std::make_pair(  "FrenchTransferReceipt", (PubsubFunctionPtr)( &FrenchTransferReceipt::subscribe )  )   );
	getClassNameUnsubscribeMap().insert(   std::make_pair(  "FrenchTransferReceipt", (PubsubFunctionPtr)( &FrenchTransferReceipt::unsubscribe )  )   );

	getDatamemberClassNameVectorPtrMap().insert(  std::make_pair( "FrenchTransferReceipt", &getDatamemberNames() )  );
	getAllDatamemberClassNameVectorPtrMap().insert(  std::make_pair( "FrenchTransferReceipt", &getAllDatamemberNames() )  );
...
std::ostream &operator<<( std::ostream &os, const FrenchTransferReceipt &entity ) {
	return os << "FrenchTransferReceipt("  << "actualLogicalGenerationTime:" << entity.get_actualLogicalGenerationTime() << ", " << "federateFilter:" << entity.get_federateFilter() << ", " << "id:" << entity.get_id() << ", " << "originFed:" << entity.get_originFed() << ", " << "sourceFed:" << entity.get_sourceFed()	<< ")";
}
...
FrenchTransferReceipt::ParameterHandleValuePairSetSP FrenchTransferReceipt::createDatamemberHandleValuePairSet( RTI::ULong count ) {
