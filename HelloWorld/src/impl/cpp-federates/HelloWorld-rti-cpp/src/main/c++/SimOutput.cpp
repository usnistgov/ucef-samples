...
void SimOutput::init( RTI::RTIambassador *rti ) {
	...
	while( isNotInitialized ) {
		try {
			getHandle() = rti->getInteractionClassHandle( "InteractionRoot.C2WInteractionRoot.SimOutput" );
	...
	getClassNameHandleMap().insert(  std::make_pair( "SimOutput", get_handle() )  );
	getClassHandleNameMap().insert(  std::make_pair( get_handle(), "SimOutput" )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "SimOutput,data", get_data_handle() )  );
...
void SimOutput::publish( RTI::RTIambassador *rti ) {
	if ( getIsPublished() ) {
		return;
	}
...
void SimOutput::unpublish( RTI::RTIambassador *rti ) {
	if ( !getIsPublished() ) {
		return;
	}
...
void SimOutput::subscribe( RTI::RTIambassador *rti ) {
	if ( getIsSubscribed() ) {
		return;
	}
...
void SimOutput::unsubscribe( RTI::RTIambassador *rti ) {
	if ( !getIsSubscribed() ) {
		return;
	}
...
bool SimOutput::static_init( void ) {
	static bool isInitialized = false;
	if ( isInitialized ) {
		return true;
	}
	...
	getClassNameSet().insert( "SimOutput" );
	
	getClassNameFactoryMap().insert(  std::make_pair( "SimOutput", &SimOutput::factory )  );
	getClassNamePublishMap().insert(   std::make_pair(  "SimOutput", (PubsubFunctionPtr)( &SimOutput::publish )  )   );
	getClassNameUnpublishMap().insert(   std::make_pair(  "SimOutput", (PubsubFunctionPtr)( &SimOutput::unpublish )  )   );
	getClassNameSubscribeMap().insert(   std::make_pair(  "SimOutput", (PubsubFunctionPtr)( &SimOutput::subscribe )  )   );
	getClassNameUnsubscribeMap().insert(   std::make_pair(  "SimOutput", (PubsubFunctionPtr)( &SimOutput::unsubscribe )  )   );

	getDatamemberClassNameVectorPtrMap().insert(  std::make_pair( "SimOutput", &getDatamemberNames() )  );
	getAllDatamemberClassNameVectorPtrMap().insert(  std::make_pair( "SimOutput", &getAllDatamemberNames() )  );
...
std::ostream &operator<<( std::ostream &os, SimOutput::SP entitySP ) {
	return os << *entitySP;
}
std::ostream &operator<<( std::ostream &os, const SimOutput &entity ) {
	return os << "SimOutput("  << "actualLogicalGenerationTime:" << entity.get_actualLogicalGenerationTime() << ", " << "data:" << entity.get_data() << ", " << "federateFilter:" << entity.get_federateFilter() << ", " << "originFed:" << entity.get_originFed() << ", " << "sourceFed:" << entity.get_sourceFed()	<< ")";
}
...
SimOutput::ParameterHandleValuePairSetSP SimOutput::createDatamemberHandleValuePairSet( RTI::ULong count ) {
