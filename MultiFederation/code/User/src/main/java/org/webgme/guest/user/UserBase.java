package org.webgme.guest.user;

import org.webgme.guest.user.rti.*;
...
public class UserBase extends SynchronizedFederate {
    private SubscribedInteractionFilter _subscribedInteractionFilter =
        new SubscribedInteractionFilter();

    // constructor
    public UserBase(FederateConfig config) throws Exception {
        super(config);
        super.createLRC();
        ...
        Config.publish_max();
        Config.publish_min();
        Config.publish(getLRC());
        Report.subscribe_average();
        Report.subscribe(getLRC());
    }
