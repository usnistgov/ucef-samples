...
void StopNodeAttack::init( RTI::RTIambassador *rti ) {
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "StopNodeAttack,recordingNodeFullPath", get_recordingNodeFullPath_handle() )  );
...
std::ostream &operator<<( std::ostream &os, const StopNodeAttack &entity ) {
	return os << "StopNodeAttack("  << "actualLogicalGenerationTime:" << entity.get_actualLogicalGenerationTime() << ", " << "federateFilter:" << entity.get_federateFilter() << ", " << "originFed:" << entity.get_originFed() << ", " << "recordingNodeFullPath:" << entity.get_recordingNodeFullPath() << ", " << "sourceFed:" << entity.get_sourceFed() << ", " << "uniqueCommandID:" << entity.get_uniqueCommandID()	<< ")";
}
